...
class Element:
    ...
    def bending_moments(self, u_global, num_points=2):
        ...
        M = (-l ** 5.0 * q + 6.0 * l ** 4.0 * q * local_x - 6.0 * q * local_x * local_x * l ** 3.0 - 48.0 * (phi_1 + phi_2 / 2.0) * EI * l ** 2.0 + 72.0 * EI * ((phi_1 + phi_2) * local_x + w_1 - w_2) * l - 144.0 * local_x * EI * (w_1 - w_2)) / 12.0 / l ** 3.0 
    ...
    def plot_displaced(self, u_elem, num_points=10, global_c=False, scale=1.0):
        ...
        Args:
            u_global (numpy.ndarray): Global displacement vector of the element.
            num_points (int, optional): Number of points to calculate the bending moments. Default is 10.
